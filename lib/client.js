window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Whether the UTC hour right now falls inside a peak window (live status for the indicator dot). */
		function isPeakNow(peakHoursUtc, now = Date.now()) {
			if (!Array.isArray(peakHoursUtc) || peakHoursUtc.length === 0) return false;
		...
		*/
		function TokenCostPill({ sessionId, useProjection, t }) {
			...
			const [peakNow, setPeakNow] = react.useState(() => isPeakNow(cost?.peakHoursUtc));
			const rootRef = react.useRef(null);
			const tokens = cost?.tokens;
			const hasData = cost !== undefined && tokens !== undefined && (tokens.uncachedInput > 0 || tokens.output > 0);
			(0, react.useEffect)(() => {
				if (!hasData) return;
				const update = () => setPeakNow(isPeakNow(cost?.peakHoursUtc));
				update();
				const timer = setInterval(update, 30_000);
				return () => clearInterval(timer);
			}, [hasData, cost?.peakHoursUtc]);
			(0, react.useEffect)(() => {
