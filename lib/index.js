 ...
 * DeepSeek peak/off-peak pricing applied by the event's LOCAL hour (default
...
const tierSchema = z.object({
  inputTokensAbove: z.number().min(1),
  input: z.number().min(0),
  cacheRead: z.number().min(0),
  cacheWrite: z.number().min(0),
  output: z.number().min(0),
...
const viewSchema = zod.object({
  ...
  peakHours: zod.array(zod.tuple([zod.number(), zod.number()])),
}).strict();
...
/**
 * Whether the LOCAL hour of a timestamp falls inside any [start, end) peak
 * window. Windows are written in the machine's local timezone (Beijing time
 * for Chinese users): DeepSeek peak = 09:00–12:00 and 14:00–18:00 local.
 */
...
function specOf(config, current) {
  if (current !== null) {
    ...
    if (current.provider.startsWith('deepseek') || current.model.startsWith('deepseek-')) return config.default;
    return null;
  }
...
function effectiveRates(spec, timeMs, windows, inputTokens = 0) {
  const tier = [...(spec.tiers ?? [])]
    .sort((a, b) => a.inputTokensAbove - b.inputTokensAbove)
    .filter((candidate) => inputTokens >= candidate.inputTokensAbove)
    .at(-1);
  const base = tier ?? spec;
  const peak = isPeakHour(timeMs, windows) && base.peak !== undefined;
  const rates = peak
    ? { input: base.peak.input, cacheRead: base.peak.cacheRead, cacheWrite: base.peak.cacheWrite, output: base.peak.output }
    : { input: base.input, cacheRead: base.cacheRead, cacheWrite: base.cacheWrite, output: base.output };
  return { rates, peak };
}
...
function costProjectionDefinition(config, peakWindows) {
  ...
  return {
    ...
    apply: (state, event) => {
      ...
      const spec = specOf(config, state.current);
      const unknownRoute = spec === null;
      const { rates, peak } = unknownRoute
        ? { rates: { input: 0, cacheRead: 0, cacheWrite: 0, output: 0 }, peak: false }
        : effectiveRates(spec, event.time ?? Date.now(), peakWindows, buckets.uncachedInput + buckets.cacheRead);
...
function normalizeSpec(spec, fallback) {
  ...
  const peak = spec.peak === undefined || spec.peak === null
    ? undefined
    : {
  ...
  const tiers = (spec.tiers ?? []).map((tier) => ({
    ...
    cacheRead: tier.cacheRead ?? base.cacheRead,
    cacheWrite: tier.cacheWrite ?? base.cacheWrite,
    output: tier.output ?? base.output,
  })).sort((a, b) => a.inputTokensAbove - b.inputTokensAbove);
  return { ...base, ...(peak === undefined ? {} : { peak }), tiers };
}
...
function apply(ctx, config) {
  ...
  const peakWindows = normalized.peakHours;
...
/**
 * Loader row config: currency, optional CNY cross-rate, rate table, and peak
 * windows in LOCAL time. Defaults are DeepSeek's peak hours for Chinese users
 * (Beijing time): 09:00–12:00 and 14:00–18:00. Set `[]` to disable peak
 * pricing entirely (the indicator then stays green).
 */
apply.Config = z.object({
  ...
  peakHours: z.array(z.tuple([z.number(), z.number()])).default([[9, 12], [14, 18]]),
});
